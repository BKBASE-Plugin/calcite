...
public class SqlLambdaScope extends ListScope {
  ...
  @Override public SqlQualified fullyQualify(SqlIdentifier identifier) {
    ...
    for (SqlNode parameter : lambdaExpr.getParameters()) {
      if (parameter.equalsDeep(identifier, Litmus.IGNORE)) {
        found = true;
        break;
      }
    }
    if (found) {
      ...
      // todo 待扩展报错类型 paramNotFoundInLambdaExpression
      throw validator.newValidationError(identifier,
          RESOURCE.columnNotFound(identifier.toString(), Locale.getDefault()));
